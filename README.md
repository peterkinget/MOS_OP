# MOS_OP - MOS Operating Point Analysis Tools

Tools for analyzing MOS operating point data from Cadence simulations.
...
```bash
# For MOS_OP:
mos_op <dcop_file> <element_file> <device_file> [--model {bsim3,bsim4}] [--abs_ids_min MIN_CURRENT]
...
```python
from mos_op import mos_op_main, mos_op2_main

# Call the main functions
mos_op_main()  # Will use command-line arguments
mos_op2_main() # Will use command-line arguments
```

You can also run the scripts directly after cloning the repository. The package is set up so that the original script files are symbolic links to the module source files, ensuring there's only one source of truth.

# Intro

* Here are a couple of python scripts to process MOS operating point
...
MOS book, namely, $C_{xx} = \partial Q_x/\partial V_{x}$ and $C_{xy} = -\partial Q_x/\partial V_{y}$ when $x\neq y$; so with this definition, if
$C_{xy}$ was a classical two-terminal capacitor then the sign of the
capacitance would indeed be positive. Note that in spectre, $C_{xy} =
\partial Q_x/\partial V_{y}$ for all $x$ and $y$.
...
If you want to contribute to this package, you can clone the repository and install it in development mode:

```bash
git clone https://github.com/peterkinget/MOS_OP.git
cd MOS_OP
pip install -e .
```
